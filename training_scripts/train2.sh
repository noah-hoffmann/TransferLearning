...
do
  ...
  train-CGAT --gpus 12 --target "$target" --fea-path "$embedding_path" --epchs 400 --clr-period 70 --data-path prepared --val-path prepared/val --test-path prepared/test --train-percentage "$set_size" --pretrained-model ""
done
